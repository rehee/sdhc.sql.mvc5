...
namespace SDHC.Common.Services
{
  public interface IContentService : ICrudContent
  {
    ...
    IEnumerable<IContentModel> GetAllChildContent(long? parentId);
    ContentTableHtmlView GetContentTableHtmlView(long? parentId);
    IContentModel GetContent(long? id);
    ContentPostModel GetPreCreate(long? id, string fullType);
    ContentPostViewModel GetContentPostViewModel(string url);
    string GetContentFullUrl(IContentModel model);
    ContentListView GetContentListView(long? id, int parentLevel = 0);
